    ...
    if ($demo_chart_line_memory_usage.length > 0) {
        new Chart($demo_chart_line_memory_usage, {
            ...
            data: {
                ...
                datasets: [{
                    label: ' Current Week Visits',
                    data: [479, 780, 994, 398, 1345, 1101, 1469],
...
$(function($) {
    ...
    function addStyleswitcher() {
        ...
        var styles = [
            {
                file: 'assets/css/theme-indigo.css',
                color: '#3949ab',
                title: 'Indigo',
                datatitle: 'theme-indigo'
            }, {
                file: 'assets/css/theme-purple.css',
                color: '#8e24aa',
                title: 'Purple',
                datatitle: 'theme-purple'
            }, {
                file: 'assets/css/theme-cyan.css',
                color: '#00acc1',
                title: 'Cyan',
                datatitle: 'theme-cyan'
            }, {
                file: 'assets/css/theme-blue.css',
                color: '#1e88e5',
                title: 'Blue',
                datatitle: 'theme-blue'
            }, {
                file: 'assets/css/theme-teal.css',
                color: '#00796b',
                title: 'Teal',
                datatitle: 'theme-teal'
            }, {
                file: 'assets/css/theme-deep-orange.css',
                color: '#d84315',
                title: 'Deep Orange',
                datatitle: 'theme-deep-orange'
            }
        ];
        ...
        output += '<div id="styleswitcher" class="styleswitcher">';
        output += '  <a href="#" class="toggle-styleswitcher"><i class="icon fa fa-paint-brush"></i></a>';
        // output += '  <div class="styleswitcher-title">Choose color</div>';
        output += '  <div class="styleswitcher-body">';
        output += '    <ul class="colors-list">';

        for (var i = 0, l = styles.length; i < l; i++) {
            output += '<li><a class="btn-switch-style" href="#" style="background: ' + styles[i]['color'] + ' ;" title="' + styles[i]['title'] + '" data-title="' + styles[i]['datatitle'] + '"></a></li>';
            $head.append(
                '<link rel="alternate stylesheet" href="' + styles[i]['file']  + '" title="'+ styles[i]['datatitle'] +  '">'
            );
        }
    ...
    function switchStyle(title) {
        ...
        $('link[title*="theme-"]').each(function() {

            var $stylesheet = $(this),
                stylesheet_title = $stylesheet.attr('title');

            if (typeof stylesheet_title === 'undefined') return;

            // Disable All => Enable target stylesheet (Chrome Fix)
            $stylesheet.prop('disabled', true);
            $stylesheet.prop('disabled', title !== stylesheet_title);

        });
    }

    $('.btn-switch-style').click(function (e) {

        e.preventDefault();
        e.stopPropagation();

        var data_title = $(this).data('title');
        if (typeof data_title === 'undefined') return;

        switchStyle(data_title);
    });

    $('.toggle-styleswitcher').click(function (e) {

        e.preventDefault();
        e.stopPropagation();

        var $styleswitcher = $('#styleswitcher');
        if ($styleswitcher.length === 0) return;
        $styleswitcher.toggleClass('styleswitcher-open');
